...
The browser opens on a redesigned home page defined in `data/home.html`. This
version is substantially larger, comprising roughly seven hundred lines of HTML
with three hundred lines of CSS and six hundred lines of JavaScript. It retains
navigation links to `downloads.html`, `news.html` and `devtools.html` but now
includes dynamic sections generated by scripts. When installed via the net
installer, the entire `data` folder is copied to `/usr/local/share/openb` so the
pages work even when you run the browser outside the source tree. Use the
